:PROPERTIES:
:ID:       A6CB045D-486F-4989-8DB9-91B35256F1C1
:END:
#+TITLE: 工欲善其事，必先利其器：Emacs 配置篇
#+AUTHOR: Gray King
#+DATE: [2022-01-07 五 17:54]
#+HUGO_BASE_DIR: ../
* 前言
...
+ ~M-w~ 将选择内容放入 ~kill-ring~ 但不删除选择区域
+ ~C-w~ 删除选择区域并将内容放入 ~kill-ring~
+ ~M-y~ 从 ~kill-ring~ 中选择粘贴
+ ~C-y~ 将放入 ~kill-ring~ 的最后一条进行粘贴
* 准备记录笔记
...
** org-babel 结合 Graphviz 内嵌流程图
org-babel 支持执行 Orgmode 中的代码块，如果你熟悉 [[https://graphviz.org/doc/info/lang.html][DOT Language]] 想要在 Orgmode 中自己画图，可以通过 org-babel 和 Graphviz 来实现。

首先，你需要先安装 [[https://graphviz.org/download/][Graphviz]]。
...
#+end_example
保存后就会生成并引用图片，图片效果如下：
[[file:images/b-tree-dot.png][B-tree DOT]]。

当然，除了画流程图，社区还提供了 [[https://github.com/millejoh/emacs-ipython-notebook][emacs-ipython-notebook]] 可以在 Orgmode 中实现类似 Jupyter Notebook 的功能。
* 接下来？
遇到不会的 ~C-h~ 是你的好帮手（which-key 会帮你弹出所有帮助主题），常用的有：
 ...
 + ~C-h v~ 变量
 + ~C-h b~ 按键绑定
然后就去问搜索引擎、社区或身边的人，同时也欢迎大家创建 [[https://github.com/coldnight/dump-brain-with-emacs/issues][issues]] 进行讨论，掌握 Emacs 无他，但手熟尔!
* Footnotes
